...
const ShopDashboard = () => {
  ...
  const shopTypeLabel = user?.businessType || user?.shopType || 'Shop';

  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-6 md:p-10">
        ...
        <div className="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <p className="text-emerald-600 font-semibold text-sm uppercase tracking-wider mb-1">
              {shopTypeLabel.charAt(0).toUpperCase() + shopTypeLabel.slice(1)} Dashboard
            </p>
            <h1 className="text-3xl md:text-4xl font-extrabold text-slate-800">
              Welcome, {user?.name?.split(' ')[0]} 👋
            </h1>
            <p className="text-slate-500 mt-1 text-sm">{user?.shopName || 'My Shop'} · {user?.shopAddress || ''}</p>
          </div>
          <div className="flex gap-3 flex-wrap">
            <button
              onClick={() => navigate('/vendor/messages')}
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-md transition-all"
            >
              📬 My Inbox
            </button>
          </div>
