...
const DeliveryDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-purple-50 p-6 md:p-10">
      <div className="max-w-6xl mx-auto">

        {/* Header */}
        <div className="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <p className="text-violet-600 font-semibold text-sm uppercase tracking-wider mb-1">Delivery Dashboard</p>
            <h1 className="text-3xl md:text-4xl font-extrabold text-slate-800">
              Welcome, {user?.name?.split(' ')[0]} 🚴
            </h1>
            <p className="text-slate-500 mt-1 text-sm">
              {user?.vehicleType ? `${user.vehicleType.charAt(0).toUpperCase() + user.vehicleType.slice(1)} Rider` : 'Delivery Person'}
              {user?.currentLocation ? ` · ${user.currentLocation}` : ''}
            </p>
          </div>
          <button
            onClick={() => navigate('/vendor/messages')}
            className="self-start sm:self-auto bg-violet-600 hover:bg-violet-700 text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-md transition-all"
          >
            📬 My Inbox
          </button>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                ...
                <div className="mt-3 h-1 w-12 bg-violet-500 rounded-full" />
              </div>
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            {/* Recent Comments */}
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              <h2 className="text-lg font-bold text-slate-800 mb-5">Recent Customer Feedback</h2>
                      ...
                      <div key={fb._id} className="flex gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 mb-1 flex-wrap">
                            <span className="font-semibold text-sm text-slate-700">{fb.userId?.name || 'Customer'}</span>
